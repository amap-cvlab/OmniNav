...
class BaseArguments(CompatArguments, GenerationArguments, QuantizeArguments, DataArguments, TemplateArguments,
    ...
    def get_model_processor(self,
        ...
        kwargs['predict_angle'] = self.predict_angle
        kwargs['ar_loss_weight'] = self.ar_loss_weight
        kwargs['arrive_loss_weight'] = self.arrive_loss_weight
        kwargs['waypoint_direction_loss'] = self.waypoint_direction_loss
        kwargs['norm_method']  = self.norm_method
        kwargs['waypoint_number'] = self.waypoint_number
        kwargs['action_dim'] = self.action_dim
        kwargs['query_action_layer'] = self.query_action_layer
        kwargs['use_arrive_list'] = self.use_arrive_list
        kwargs['cross_attention_flow_match'] = self.cross_attention_flow_match
