...
def get_model_tokenizer_qwen2_vl(*args, **kwargs):
    ...
    #mark add
    add_special_token_flag=True
    print("------------")
    print("add_special_token?",add_special_token_flag)
    print("------------")
    if add_special_token_flag:
        # special_token_list = special_tokens.split(',')
        # # print(tokenizer)
        # tokenizer.tokenizer.add_tokens(special_token_list)
        # model.resize_token_embeddings(len(tokenizer.tokenizer))
        #151657新token开始 qwen2
        #151665新token开始 qwen2.5
        special_tokens = "<input_pos1>,<input_pos2>,<input_pos3>,<input_pos4>,<input_pos5>,<input_target>,<|forward|>,<|left|>,<|right|>,<|stop|>"
        special_token_list = special_tokens.split(',')
        # Create a dictionary for special tokens
        special_tokens_dict = {'additional_special_tokens': special_token_list}

        # Add the special tokens to the tokenizer
        num_added_tokens = tokenizer.tokenizer.add_special_tokens(special_tokens_dict)
        special_token2id = {}
        for token in special_token_list:
            special_token2id[token]=tokenizer.tokenizer(token)['input_ids'][0]
