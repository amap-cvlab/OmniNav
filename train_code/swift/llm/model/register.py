    ...
    if load_model:
        ...
        if model is None:
            if model_info.task_type == 'seq_cls' and not model_meta.is_reward:
                ...
                context = partial(patch_automodel, automodel_class=automodel_class, model_info=model_info)
            with context():
                model_config.predict_angle = kwargs['predict_angle']#mark
                model_config.ar_loss_weight = kwargs['ar_loss_weight']
                model_config.arrive_loss_weight = kwargs['arrive_loss_weight']
                model_config.waypoint_number = kwargs['waypoint_number']#mark
                model_config.action_dim = kwargs['action_dim']
                model_config.query_action_layer = kwargs['query_action_layer']#mark
                model_config.waypoint_direction_loss = kwargs['waypoint_direction_loss']#mark
                model_config.norm_method = kwargs['norm_method']#mark
                model_config.use_arrive_list = kwargs['use_arrive_list']#mark
                model_config.cross_attention_flow_match = kwargs['cross_attention_flow_match']#mark
                kwargs.pop('predict_angle')#mark
                kwargs.pop('ar_loss_weight')
                kwargs.pop('arrive_loss_weight')
                kwargs.pop('waypoint_number')#mark
                kwargs.pop('action_dim')#mark
                kwargs.pop('query_action_layer')#mark
                kwargs.pop('waypoint_direction_loss')#mark
                kwargs.pop('use_arrive_list')#mark
                kwargs.pop('cross_attention_flow_match')#mark
                kwargs.pop('norm_method')#mark
