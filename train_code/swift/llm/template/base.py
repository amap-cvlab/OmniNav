...
from .vision_utils import load_audio, load_batch, load_image, rescale_image,rescale_image_to_fixed_size, rescale_image_to_640 #mark add rescale_image_to_fixed_size, rescale_image_to_640
import numpy as np #mark
def crop_resize_image_magic_resolution(img: Image.Image) -> Image.Image:
    ...
    # PIL 的 crop 参数是 (left, top, right, bottom)
    
    # 创建 transform 将图像 resize 到原始大小
    
    # 转为 tensor -> resize -> 转回 PIL
    # 或者直接用 PIL resize 更简单
    resized_img = cropped_img.resize((width, height), Image.BILINEAR)
    
    return resized_img
def rescale_image_maxlength_to_fixed_res(img: Image.Image,res=1400) -> Image.Image:
    import torchvision.transforms as T
    width = img.width
    height = img.height
    if max(width,height) > res:
        if width > height:
            # 宽度是长边，缩放宽度到 640，高度等比例缩放
            new_width = res
            new_height = int(height * (new_width / width))
        else:
            # 高度是长边，缩放高度到 640，宽度等比例缩放
            new_height = res
            new_width = int(width * (new_height / height))
        return T.Resize((new_height, new_width))(img)
    else:
        return img
def rescale_detection_image(image: Image.Image, max_side=644):
...
def rescale_image_with_long_edge_and_random_scale(image: Image.Image, 
    ...
    """
    对图像进行两步缩放：
    1. 将长边 resize 到 long_edge，短边等比例缩放（保持 aspect ratio）
    2. 再整体乘以一个随机缩放因子 s ~ uniform(scale_range)
    
    返回缩放后的 PIL 图像
    """
    w, h = image.size
    
    # Step 1: 等比例缩放，使长边为 long_edge
    if w >= h:
        new_w = long_edge
...
class Template(ProcessorMixin):
    ...
    def __init__(
        ...
        *,
        ...
        resized_img_downsample: bool = False,#mark add
        resized_history_image: bool = False,
        resized_img_fixed: bool = False,#mark add
        dynamic_resolution: bool = False,
        magic_resolution: bool = False,
        fixed_height: int = None,#mark add
        current_img_num: int =1,#mark add
        fixed_width: int = None,#mark add
        input_waypoint_augment: bool = False,#mark add
        action_former: bool = False,#mark add
        if_arrive_list_drop_input_waypoint: bool = False,
        if_arrive_list_need_arrive_loss: bool = False,
        predict_angle: bool = False,#mark add
        ar_loss_weight: float = 1.0,
        arrive_loss_weight: float = 1.0,
        use_arrive_list: bool = False,#mark
        cross_attention_flow_match: bool = False,#mark
        waypoint_direction_loss: bool = True,#mark add
        norm_method : Optional[str] = None,
        query_action_layer: int = 1,#mark add
        waypoint_number: int = 5,
        action_dim: int =5,
        use_input_waypoint: bool = False,#mark add
        ...
        self.resized_img_downsample = resized_img_downsample#mark add
        self.resized_history_image = resized_history_image
        self.resized_img_fixed = resized_img_fixed #mark add
        self.dynamic_resolution = dynamic_resolution
        self.magic_resolution = magic_resolution
        self.current_img_num = current_img_num
        self.fixed_height = fixed_height#mark add
        self.fixed_width = fixed_width#mark add
        self.input_waypoint_augment = input_waypoint_augment #mark
        self.action_former = action_former#mark
        self.if_arrive_list_drop_input_waypoint = if_arrive_list_drop_input_waypoint#mark
        self.if_arrive_list_need_arrive_loss = if_arrive_list_need_arrive_loss
        self.predict_angle = predict_angle#mark
        self.ar_loss_weight = ar_loss_weight
        self.arrive_loss_weight = arrive_loss_weight
        self.use_arrive_list = use_arrive_list#mark
        self.cross_attention_flow_match = cross_attention_flow_match#mark
        self.waypoint_direction_loss = waypoint_direction_loss#mark
        self.norm_method = norm_method
        self.query_action_layer = query_action_layer #mark
        self.waypoint_number = waypoint_number#mark
        self.action_dim = action_dim
        self.use_input_waypoint = use_input_waypoint#mark
        self.dataset_not_concat = dataset_not_concat#mark
    ...
    def _preprocess_inputs(
        ...
        #mark add
        if 'train_branch' in inputs.messages[1] and inputs.messages[1]['train_branch']=='bbox':#'|detection|' in inputs.messages[0]['content'] or '|refering|' in inputs.messages[0]['content'] or '|grounding|' in inputs.messages[0]['content']:
            images = [rescale_detection_image(img,644) for img in images]
        elif 'train_branch' in inputs.messages[1] and inputs.messages[1]['train_branch']=='general':
            images = [rescale_image_maxlength_to_fixed_res(img,1400) for img in images]
        elif self.dynamic_resolution:
            if self.magic_resolution:
                images = [
            ...
            if self.resized_history_image:
                images = [rescale_image_to_fixed_size(img,int(img.height/4),int(img.width/4)) if idx < len(images)-self.current_img_num else img for idx,img in enumerate(images)]
        else:
            if self.resized_img_fixed:
                images = [rescale_image_to_fixed_size(img,self.fixed_height,self.fixed_width) for img in images]
            if self.resized_history_image:
                #current_img_num
                images = [rescale_image_to_fixed_size(img,int(img.height/4),int(img.width/4)) if idx < len(images)-self.current_img_num else img for idx,img in enumerate(images)]
            if self.resized_img_downsample:
                images = [rescale_image_to_640(img) for img in images]
        #mark over
    ...
    def _encode_truncated(self, inputs):
        ...
        if self.action_former:
            encoded['action_former']=True
        ###mark add
        encoded['train_branch'] = 'ar'
        if self.if_arrive_list_need_arrive_loss and 'arrive_list' in inputs.messages[1]:
            encoded['drop_arrive_loss'] = False
        else:
            encoded['drop_arrive_loss'] = True
        if 'train_branch' in inputs.messages[1] and inputs.messages[1]['train_branch']=='continue':
            if 'arrive' in inputs.messages[1]:
                encoded['arrive'] = inputs.messages[1]['arrive']
            if 'arrive_list' in inputs.messages[1]:
                encoded['arrive_list'] = inputs.messages[1]['arrive_list']
            if 'norm' in inputs.messages[1]:
                encoded['norm'] = inputs.messages[1]['norm']
            encoded['gt_waypoints'] = inputs.messages[1]['gt_waypoints']
            if 'gt_heading_angles' in inputs.messages[1]:
                encoded['gt_heading_angles'] = inputs.messages[1]['gt_heading_angles']
            encoded['train_branch'] = 'continue'
            #inputs.messages[1]['input_waypoints'][-1]=[0.0,0.0]
            if self.if_arrive_list_drop_input_waypoint and 'arrive_list' in inputs.messages[1]:
                pass
            elif self.input_waypoint_augment and self.use_input_waypoint:
                prob = np.random.uniform(0,1)
                if prob <0.5:
                    encoded['input_waypoints'] = inputs.messages[1]['input_waypoints']
                elif prob <0.7:
                    input_waypoints = inputs.messages[1]['input_waypoints']
                    distrub = (np.random.randn(4,2) * 0.2).tolist()+[[0,0],[0,0]]#0515
                    input_waypoints = (np.array(input_waypoints) + np.array(distrub)).tolist()
                    encoded['input_waypoints'] = input_waypoints
                elif prob < 0.8:
                    input_waypoints = inputs.messages[1]['input_waypoints']
                    distrub = (np.random.randn(4,2) * 1).tolist()+[[0,0],[0,0]]#0515
                    input_waypoints = (np.array(input_waypoints) + np.array(distrub)).tolist()
                    encoded['input_waypoints'] = input_waypoints                
                else:
                    input_waypoints = inputs.messages[1]['input_waypoints']
                    distrub = (np.random.randn(4,2) * 0.02).tolist()+[input_waypoints[-2],input_waypoints[-1]]
                    encoded['input_waypoints'] = distrub
            else:
                if 'input_waypoints' in inputs.messages[1] and self.use_input_waypoint:
                    encoded['input_waypoints'] = inputs.messages[1]['input_waypoints']
        else:
            pass
        ###mark add over
    ...
    def pre_forward_hook(self, model: nn.Module, args, kwargs):
        ...
        for k, v in old_kwargs.items():
            if k in {'input_ids', 'attention_mask', 'labels', 'position_ids', 'output_hidden_states', 'logits_to_keep','arrive','gt_waypoints','input_waypoints','action_former','train_branch','arrive_list','norm','gt_heading_angles','drop_arrive_loss'
                     } and k not in kwargs:#mark add
                kwargs[k] = v
        #mark change
        #before
        ...
        #cur
        if 'inputs_embeds' in kwargs and 'gt_waypoints' not in kwargs:
            kwargs.pop('input_ids', None)        
        #mark change over
    ...
    def _data_collator(self, batch: List[Dict[str, Any]], *, padding_to: Optional[int] = None) -> Dict[str, Any]:
        ...
        if packing_mode:
            ...
            #         res[key] = val
            #cur
            for key in ['labels', 'loss_scale', 'position_ids', 'token_type_ids','arrive','arrive_list','gt_waypoints','input_waypoints','action_former','train_branch','norm','gt_heading_angles','drop_arrive_loss']:#mark add arrive gt_waypoints,action_former
                val = [b[key] for b in batch if b.get(key) is not None]
                if val and (key=='arrive' or key=='gt_waypoints' or key == 'input_waypoints' or key == 'arrive_list' or key == 'gt_heading_angles'):
                    res[key] = torch.tensor(val)
                if val and key!='arrive' and key!='gt_waypoints' and key!='input_waypoints' and key != 'arrive_list' and key != 'gt_heading_angles':
                    res[key] = val
            #mark over
