...
class SwiftSft(SwiftPipeline, TunerMixin):
    ...
    def _get_dataset(self):
        ...
        train_dataset, val_dataset = load_dataset(
            args.dataset, split_dataset_ratio=args.split_dataset_ratio, shuffle=args.dataset_shuffle, dataset_not_concat = args.dataset_not_concat, **dataset_kwargs)
    ...
    def run(self):
        ...
        #cur
        if args.dataset_not_concat:
            new_train_dataset = []
            _, val_dataset = self._encode_dataset(None, val_dataset)
            for cur_dataset in train_dataset:
                cur_train_dataset, _ = self._encode_dataset(cur_dataset, None)
                new_train_dataset.append(cur_train_dataset)
        else:
            train_dataset, val_dataset = self._encode_dataset(train_dataset, val_dataset)
        #mark over
        ...
        #cur
        if args.dataset_not_concat:
            self.model = self.prepare_model(self.args, self.model, template=self.template, train_dataset=new_train_dataset)
        else:
            self.model = self.prepare_model(self.args, self.model, template=self.template, train_dataset=train_dataset)
        #mark over
        ...
        trainer = trainer_cls(
            ...
            train_dataset=train_dataset if not args.dataset_not_concat else new_train_dataset,
    ...
    def _encode_dataset(self, train_dataset, val_dataset):
        ...
        if not is_grpo:
            ...
            if val_dataset is not None and hasattr(val_dataset, '__len__') and len(val_dataset) == 0:
                val_dataset = None
            if train_dataset is not None and isinstance(train_dataset, (HfDataset, PackingDataset)):
